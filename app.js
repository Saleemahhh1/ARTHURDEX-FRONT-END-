// app.js (ArthurDex frontend - improved, crash-proof)
// -> Ensure BACKEND_URL does NOT end with "/api".
//    You can override by setting window.__BACKEND_URL__ = "https://your.url" before this script runs,
//    or edit the constant below.

const PROJECT_ID = "031c9ba70da1c67907ed7484f7a6aa64";
const BACKEND_URL = (window.__BACKEND_URL__ || "https://arthurdex.onrender.com").replace(/\/+$/,'');

/* ---------------- small helpers ---------------- */
const safe = (fn) => { try { return fn(); } catch(e){ console.warn('safe() caught', e); return null; } };
const $ = id => document.getElementById(id);

/* ---------------- guarded fetch (auto-prefix & timeout & JSON safe) ---------------- */
async function guardedFetch(rawUrl, opts = {}, timeout = 12000) {
  // Build absolute URL if caller passed a relative path (starting with /)
  let url = rawUrl;
  if (typeof rawUrl === 'string' && rawUrl.startsWith('/')) {
    // prefix with BACKEND_URL
    url = `${BACKEND_URL}${rawUrl}`;
  } else if (typeof rawUrl === 'string' && !/^https?:\/\//i.test(rawUrl)) {
    // non-http non-leading-slash — treat as relative as well
    url = `${BACKEND_URL}/${rawUrl}`.replace(/\/+/g,'/');
  }
  console.debug('[guardedFetch] calling', url, opts);
  ...
  try {
    ...
    clearTimeout(id);
    if (!res.ok) {
      // try to parse response text/json for better error message
      let txt;
    ...
    clearTimeout(id);
    console.warn('[guardedFetch] error', url, err && err.message ? err.message : err);
...
document.addEventListener('DOMContentLoaded', () => {
  // Query elements AFTER DOM ready, guard each
  ...
  // small helpers that use closures above
  ...
  function modalClose(){ if(!modalEl) return; modalEl.classList.add('hidden'); modalEl.innerHTML=''; modalEl.setAttribute('aria-hidden','true'); }
  ...
  async function runIntro(){
    try{
      if(!introLine || !introScreen || !termsScreen || !powered) {
        // If missing, show options directly
  ...
  /* ---------------- UI hookups: attach listeners safely ---------------- */
  // helper to safely add event if element exists
  ...
  /* ---------------- flows (same as your logic, slightly defensive) ---------------- */
  ...
  async function showPasswordCreationAndRegister(passphrase){
    ...
    btn.onclick = async ()=>{
      ...
      // Use relative path beginning with /api -> guardedFetch will prefix BACKEND_URL
  ...
  /* ---------------- WalletConnect / HashPack (unchanged but defensive) ---------------- */
  ...
  async function goToMainDashboard(){
    ...
    try{
      if(active){
        // call backend balance route (relative path -> guardedFetch prefixes BACKEND_URL)
        ...
        if(r.ok && r.data){
          ...
          const hbarChangeEl = $('hbarChange');
          if(hbarChangeEl){ hbarChangeEl.textContent = pct>=0 ? `+${pct}%` : `${pct}%`; hbarChangeEl.style.color = pct>=0 ? '#9fffcc' : '#ff9f9f'; }
        } else {
  ...
  // send modal (keeps local demo fallback)
